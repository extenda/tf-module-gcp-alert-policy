...
resource "google_monitoring_alert_policy" "alert_policy" {
  ...
  dynamic "conditions" {
    ...
    content {
      ...
      dynamic "condition_threshold" {
        ...
        content {
          ...
          dynamic "forecast_options" {
            for_each = try([condition_threshold.value.forecast_options], [])
            content {
      ...
      dynamic "condition_prometheus_query_language" {
        for_each = try([conditions.value.condition_prometheus_query_language], [])
        content {
          query               = try(condition_prometheus_query_language.value.query, "")
          duration            = try(condition_prometheus_query_language.value.duration, local.default_duration)
          evaluation_interval = try(condition_prometheus_query_language.value.evaluation_interval, local.default_evaluation_interval)
          labels              = try(condition_prometheus_query_language.value.labels, {})
          rule_group          = try(condition_prometheus_query_language.value.rule_group, "")
          alert_rule          = try(condition_prometheus_query_language.value.alert_rule, "")
        }
      }

      dynamic "condition_absent" {
        for_each = try([conditions.value.condition_absent], [])
        content {
          duration = condition_absent.value.duration
          filter   = try(condition_absent.value.filter, "")

          dynamic "trigger" {
            for_each = try([condition_absent.value.trigger], [])
            content {
  ...
  lifecycle {
    precondition {
      # Look up the errors for THIS specific policy
      condition     = length(local.policy_errors_by_key[each.key]) == 0
      
      # Print only the errors relevant to this policy
